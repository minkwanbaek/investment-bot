...
def build_recent_ralph_log(ralph_dir: Path, recent_count: int) -> None:
    log_path = ralph_dir / 'RALPH_LOG.md'
    recent_path = ralph_dir / 'RECENT_RALPH_LOG.md'
    if not log_path.exists():
        recent_path.write_text('[RECENT_RALPH_LOG]\n[/RECENT_RALPH_LOG]\n', encoding='utf-8')
        return

    text = log_path.read_text(encoding='utf-8')
    blocks = re.findall(r'\[RALPH_LOG\][\s\S]*?\[/RALPH_LOG\]', text)
    if len(blocks) <= 1:
        recent_blocks = blocks
    else:
        template_block = blocks[0]
        data_blocks = blocks[1:]
        recent_blocks = [template_block] + data_blocks[-recent_count:]

    body = '\n\n'.join(recent_blocks).strip()
    recent_path.write_text(body + '\n', encoding='utf-8')
...
def main() -> int:
    ...
    parser.add_argument("--recent-log-count", type=int, default=8, help="How many recent Ralph log entries to copy into RECENT_RALPH_LOG.md")
    ...
    build_recent_ralph_log(ralph_dir, recent_count=max(args.recent_log_count, 1))
    return 0
